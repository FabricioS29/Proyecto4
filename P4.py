...
# ---
# 
# * Estudiante: **Fabricio Solano Rojas**
# * Carné: **B77447**
# * Grupo: **1**
# 
# ---
...
def modulador(bits, fc, mpp):
    ...
    return senal_Tx, Pm, portadoraT, moduladora  
...
def demodulador(senal_Rx, portadoraT, mpp):
    '''Un método que simula un bloque demodulador
    ...
    # Energía de un período de la portadora
    Es = np.sum(portadoraT**2)

    # Demodulación
    for i in range(N):
        # Producto interno de dos funciones
        producto = senal_Rx[i*mpp : (i+1)*mpp] * portadoraT
        senal_demodulada[i*mpp : (i+1)*mpp] = producto
        Ep = np.sum(producto) 
...
# 3. Modular la cadena de bits usando el esquema BPSK
senal_Tx, Pm, portadoraT, moduladora = modulador(bits_Tx, fc, mpp)

# 4. Se transmite la señal modulada, por un canal ruidoso
senal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)

# 5. Se desmodula la señal recibida del canal
bits_Rx, senal_demodulada = demodulador(senal_Rx, portadoraT, mpp)
...
# Promedio de las 4 realizaciones en cada instante (cada punto en t)
...
# Como lo muestra la gráfica de Realizaciones del proceso aleatorio se puede observar que el promedio de Realizaciones
# y el valor teórico presentan el mismo valor, por lo tanto, en esta señal modulada senal_Tx hay ergodicidad.
...
plt.xlabel('$t$')
plt.ylabel('señal modulada (senal_Tx)')
plt.show()
